...
fn tm_value_primitives() {
    ...
    let first_value_copy = u32::from_bytes(&first_value_bytes).unwrap();
...
fn tm_value_structs() {
    ...
    let second_value = TestVector { x: 3, y: 3.3, z: TestValue { val: Some(1) }};
    
    let first_value_bytes = first_value.to_bytes();
    let second_value_bytes = second_value.to_bytes();

    let first_value_copy = TestValue::from_bytes(&first_value_bytes).unwrap();
    let second_value_copy = TestVector::from_bytes(&second_value_bytes).unwrap();
...
fn tm_value_arrays() {
    ...
    let first_value_copy = <[i32; 7]>::from_bytes(&first_value_bytes).unwrap();
...
fn tm_value_enums() {
    ...
    let first_value_bytes: [u8; 1+1+4] = first_value.to_bytes();
    let first_value_copy = TestEnum::from_bytes(&first_value_bytes).unwrap();
